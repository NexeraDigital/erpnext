...
- Brandon's original pilot did **not** integrate a real OCR provider — the "OCR" was a deterministic hash function (same input always yields the same output; makes tests reproducible). **This is now changing:** later work added a provider adapter (Phase 1) and a real Anthropic Claude extractor (Phase 2) that reads an actual invoice image/PDF and returns the supplier, invoice number, date, total, and currency. The fake provider is still the default until the settings toggle lands (Phase 3); the real one is opt-in and key-gated. See `docs/planning/real-ocr-implementation-plan.md`.
