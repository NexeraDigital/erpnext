...
> **Status (2026-05-31):** implemented + tested on `russ/migrateToV16` (working tree). Fourth slice of the v2 build — see `docs/spec/04-extraction-confidence-line-items.md`. All 15 acceptance criteria green this session; **13 new automated tests** pass; capture suite now **86 OK** (was 81), extractors **19 OK** (was 15), anthropic **26 OK** (was 23), settings **17 OK** (was 16); hardening/audit/integration (16/11/5) and spec-01/02/03 suites pass unchanged.
...
```
 ...
 erpnext/accounts/doctype/ap_invoice_capture/ap_invoice_capture.json                                       | +/- extraction_detail_section + subtotal_amount/tax_amount (Currency) + line_items (Table) + field_confidences (Table)
