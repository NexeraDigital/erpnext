...
>
> **Last updated:** 2026-05-31 · **Specs:** 14 · **Acceptance criteria:** 221 · **Overall:** 🟡 4/14 done — **specs 01–04 ✅** (63/221 ACs green; 69 new tests passing; no regressions)
...
| 04 | [[04-extraction-confidence-line-items]] | 1 Intake | ✅ | 15/15 | L | 01 | #4 locked | local ✓ (uncommitted) |
...
> **Verification state (04):** ✅ all 15 ACs green this session — `test_ap_invoice_capture` **Ran 86 tests … OK**, `test_extractors` **19 OK**, `test_anthropic` **26 OK**, `test_ap_closed_loop_settings` **17 OK**; no-regression proof: `test_hardening`/`test_audit`/`test_integration` (16/11/5) + spec-01/02/03 suites unchanged. The §7.2 clean-room runbooks (real-Anthropic per-field confidence + line-items-promote) are **written but not executed** (need a live API key); the §7.3 Playwright pass is pending. The per-field threshold reused spec-01's `field_thresholds` + canonical scalar (no second scalar — locked decision #4).
>
