...
>
> **2026-06-04 — content-based receipt/invoice classifier (spec 07 §34).** Step-6 classification can now read receipt-vs-invoice from the **document content** instead of the filename/keyword markers: Phase 1 a deterministic rule scorer, Phase 2 an optional LLM, behind one dispatcher that degrades to the rule scorer on any LLM failure. **Default OFF** (`enable_content_classifier`); confidence + rationale recorded either way. **20 new tests** green this session (`TestAPContentClassifier` 8 + `TestAPContentClassifierLLM` 6 + the receipt-corpus harness); shipped behaviour byte-identical until opted in. Measured on a 42-doc labeled corpus (`test/receipts/`): rule 62% / **LLM 88%** genre, semantic tier 50%→**100%**. FORK-CHANGES §34; sequence diagram refreshed (Step 1b). Phases 3 (hybrid) + 4 (real-OCR accuracy) → TODO T-024/T-025.
>
...
>
> **2026-06-02 — Spec 13 built ✅ (bank-feed match = the external close signal).** New capture fields `bank_cleared` (Check) + `bank_transaction` (Link → Bank Transaction); `reconcile_bank_for` reads the **native** Bank Reconciliation result (Bank Transaction Payments) and stamps `bank_cleared` + the matched BT + `payment_lifecycle_status='Bank Cleared'` when the bank feed has matched a Bank Transaction to the capture's Payment Entry. **Closure is now dual-signal:** `build_closure_evidence` exposes `settled` (internal: PI Paid + PE submitted) + `bank_cleared` (external), and `closed = settled AND bank_cleared`. The **Phase-1 'no Bank Transaction' guardrail is retired** — a Bank Transaction now *enables* closure rather than breaking it (the two closure-evidence tests + walking-skeleton expectations updated). `TestAPBankReconciliation` **`Ran=4 … OK`** (incl. a real Bank Transaction match) + 2 updated closure tests; full module **`Ran=217 … OK`** (zero regressions). **Deferred to cutover:** the live feed connection (native **Plaid** default per decision #8); dev uses fixture Bank Transactions. Stream-R JE bank-match is a noted edge (already-paid posts an is_paid PI, not a separate JE).
>
