...
| `target_supplier` | Link | → Supplier | empty for Create; populated for Update/Disable variants |
| `proposed_payload` | Code | options `JSON` | Supplier fields to write on approval (see payload contract below) |
...
5. For `change_type == "Create"`: `frappe.get_doc({"doctype": "Supplier", **allowlisted(proposed_payload)}).insert()`; set `req.created_supplier`, `decision_by`, `decision_at`, `workflow_state="Posted"`. (Update Bank Details / Payment Terms / Disable variants write to `target_supplier` instead — bodies owned by [[08-validation-gates]]/[[11-approval-sod-workflow]], but the dispatch + SoD live here.)
