...
> **Flows with no human:** when an `AP Supplier Coding Profile` resolves, expense account / cost center / tax / terms are auto-filled onto the draft PI and the document flows straight through — `is_fully_coded` goes True with no touch. Stream R falls back to the Unmapped Card Spend catch-all and posts without pausing.
> **Single escalation seam:** **genuine cost-center or tax ambiguity → Coding Review** (`coding_status` Ambiguous/Flagged → the capture parks in the coding-review queue). Two signals resolving to different cost centers, or extracted tax disagreeing with the template beyond tolerance, are the only things that pull a human — a real decision a human must own. Everything unambiguous auto-codes.
> **Automation growth target:** today auto-coding fires **only when a human pre-built a profile** — a new-but-routine vendor is hand-coded every single time until someone sets one up, and there is **no learning from history**. The target is **"Coding bootstrap from history" (§5.3.1, planned)**: auto-derive the coding suggestion (expense account / cost center / tax) from the vendor's **prior posted Purchase Invoices**, apply it automatically when the history is consistent and confident, and escalate to Coding Review only on genuine ambiguity or no history — so the profile **bootstraps itself** instead of requiring human setup.

## 1. Summary
This spec builds the **auto-coding layer** for routine vendors: a new per-supplier master `AP Supplier Coding Profile` that layers default expense account, cost center, purchase-tax template, payment terms, and accounting dimensions on top of native Party Account, plus a re-runnable `apply_coding_profile_for(capture)` entry point that merges three default layers and writes them onto the **draft** Purchase Invoice. It implements **workflow-v2-plan.md Step 5** for **both streams** — Stream I (full coding before approval) and Stream R (catch-all "Unmapped Card Spend" expense when no profile resolves). Current-state delta: today the promote path does a two-layer merge (`_coalesce_defaults`, document_capture.py:1116) with **no supplier tier, no cost-center inference, no tax, and no submitted-PI guard** — this spec adds all four and exposes `is_fully_coded(capture)` as the gate [[08-validation-gates]] / [[09-confidence-routing]] consult before anything auto-posts.
...
- **Two-layer defaults merge — NO supplier tier.** `_coalesce_defaults(defaults)` at `erpnext/accounts/doctype/document_capture/document_capture.py:1116-1145` merges **only** `AP Closed Loop Settings` (lowest, via `get_promote_defaults()`) then the caller-supplied `defaults` dict (highest, non-None wins per-field, lines 1140-1143). It filters the result to `_PROMOTE_DEFAULT_KEYS = ("company","item_code","qty","uom","warehouse","expense_account","cost_center")` (lines 1105-1113). The per-supplier middle tier this spec adds **does not exist**.
- **Promote writes coding verbatim, no inference, no tax.** `promote_to_purchase_invoice(capture, actor, defaults, save)` at `document_capture.py:1148-1235` builds **one** PI item row and sets `item_row["expense_account"]` (1213-1214) and `item_row["cost_center"]` (1215-1216) **only if present in the coalesced defaults**, taken verbatim — there is **no** cost-center inference from location / card-last4 / supplier, **no** ambiguity→review branch, and **no** tax handling at all (no `pi.taxes_and_charges`, no `pi.taxes` rows, no Party Account / payable logic).
...
- **Cascade is a pure state machine.** `_determine_next_step` (`document_capture.py:321-366`) returns `(method, reason)` and currently has **no coding hop** — validation (Step 2) flows straight to the manual promote seam, then promotion → approval (Step 3). This spec inserts the coding step (§5.4).
...
|---|---|---|---|
| C1 | https://docs.frappe.io/erpnext/user/manual/en/cost-center | Cost Center is an org unit costs/income are charged to; settable at **Company / Item / Order-Invoice** levels — grounds the per-supplier `default_cost_center` and the PI-item `cost_center` the promote path writes. The page does **not** name the PI Item field or GL mechanics — for those the in-repo code (`document_capture.py:1215-1216`, `purchase_invoice_item.json:525`) and the `accounting_dimension_doctypes` hook are source of truth. | "A Cost Center is a part of an organization where costs or income can be charged." Levels listed: "Company," "Item," "Order/Invoice." |
...
#### 5.1.4 Changed DocType: `Document Capture` — new fields
...
| `coding_source` | Data | default `ap-coding-v1` | Provenance tag (mirrors `validation_source`, `document_capture.py:1081`). | this spec |

The existing `action_required` / `action_required_reason` pair (used by `validate_for_purchase_invoice`, `document_capture.py:1086-1089`) is **reused** to route ambiguous/flagged coding to review — `coding_status` is the structured signal, `action_required` the queue trigger.

### 5.2 Endpoints

All whitelisted methods are co-located in `document_capture.py`, mirror the existing `*_for` wrapper convention (e.g. `promote_to_purchase_invoice_for`, `document_capture.py:1687`; `validate_for_purchase_invoice_for`, `:1678`), accept a capture **name (str) OR doc** like every sibling, normalize a `defaults` arg from JSON-string-or-dict exactly as `promote_to_purchase_invoice_for` does (`:1692-1696`), and call `doc._kick_next_step()` before returning the name to resume the cascade.
...
1. **Normalize input.** If `capture` is a str, `frappe.get_doc("Document Capture", capture)` (mirrors `:1038`).
...
**Design (a new Layer-1.5, between profile and Settings).** Add a pure helper `_derive_coding_from_history(matched_supplier, *, company=None) -> dict` that, when no profile resolves (or to fill keys the profile leaves empty), inspects the supplier's **prior posted Purchase Invoices**:

1. **Source set.** `frappe.get_all("Purchase Invoice", filters={"supplier": matched_supplier, "docstatus": 1, "company": company or settings.default_company}, ...)`, most-recent-first, capped (e.g. last `N=10`, tunable via Settings). Join their PI Item rows for `expense_account` / `cost_center` and the header `taxes_and_charges`. Cancelled/draft PIs are excluded (only `docstatus==1` counts as ground truth).
2. **Consistency test (the confidence gate).** For each codable field independently — `expense_account`, `cost_center`, `taxes_and_charges` — compute the distribution across the source set. A field is **confidently derived** iff a single value covers `>= settings.coding_history_consensus_threshold` (default e.g. 0.8) of the non-empty observations **and** the sample count `>= settings.coding_history_min_samples` (default e.g. 3). Otherwise that field is **not** derived (left for escalation).
3. **Apply automatically when confident.** Confidently-derived fields are merged into the coding layers **below the profile and the caller `defaults`** but **above Layer-0 Settings** (so an explicit profile or caller override always wins; history only fills what a human hasn't pinned). When every field `is_fully_coded` needs is confidently derived → the capture auto-codes (`coding_status="Coded"`) and the cascade advances with **no human and no pre-built profile**.
4. **Escalate only on genuine ambiguity or no history.** If a field's history is **inconsistent** (no value clears the consensus threshold) → that field is **not** auto-written and `coding_status` lands `Ambiguous`/`Flagged` with `coding_review_reason` naming the field and its competing values → the capture parks in **Coding Review** (the same seam §5.4 already defines). If there is **no qualifying history** (sample count below `coding_history_min_samples`) and no profile → fall through to today's behavior (Settings/Stream-R catch-all, or Coding Review). The escalation seam is unchanged; history only *removes* avoidable escalations, it never creates new auto-posts on shaky evidence.
5. **Cost-center interaction.** History-derived `cost_center` is treated as **one more inference signal** inside `_infer_cost_center` (§5.3 step 5), at a priority **below** the live capture signals (location, card-last4) but usable as the supplier-axis default when the explicit `profile.default_cost_center` is absent. A history CC that *conflicts* with a live location/card signal triggers the existing ambiguity → Coding Review path (no silent pick — same R5 discipline).
6. **Optional profile self-seeding (decision, see D9).** When history confidently derives a full coding set, the system MAY auto-create/update the `AP Supplier Coding Profile` for that supplier (provenance-tagged) so subsequent captures resolve at the cheaper Layer-1 lookup and the derived defaults become visible/editable to operators — the profile literally bootstraps itself. Recommended: yes, write a profile flagged `coding_source="ap-coding-history-v1"`, leaving operators free to curate it.

**New Settings keys (planned, `AP Closed Loop Settings`):** `enable_coding_history_bootstrap` (Check, default — see D9), `coding_history_consensus_threshold` (Float, default 0.8), `coding_history_min_samples` (Int, default 3), `coding_history_lookback` (Int, default 10). Surfaced via the same `get_singles_dict` raw-read discipline (risk R4).

**Grounding to verify at build:** the PI Item field names (`expense_account` `purchase_invoice_item.json:491`, `cost_center` `:525`) and the PI header `taxes_and_charges` (`purchase_invoice.json:748-753`) are already cited in §3/§4; the history query touches only those native fields plus `docstatus`/`supplier`/`company` on `Purchase Invoice`. No new framework surface — re-confirm the field names against the installed app before coding per the grounding rule.

### 5.4 Cascade & stream-awareness

- **New hop in `_determine_next_step`** (`document_capture.py:321-366`), inserted **between** the validation step (current Step 2, line 339) and the manual promote seam:
...
- **Async / enqueue:** the coding hop routes through the existing `_enqueue_next` / `_run_cascade_step` wrapper (`document_capture.py:368-403`) — `enqueue_after_commit=True`, `deduplicate=True`, per-capture-per-step job; under `in_test` it runs `now=True`. No new queue plumbing.
...
- **`erpnext.accounts.doctype.document_capture.test_document_capture`** (extend existing, `test_document_capture.py`):
...
| D9 | **Coding-bootstrap-from-history default & confidence knobs** *(added 2026-06-02; the automation-first slice, §5.3.1)* | (a) leave it off / opt-in (`enable_coding_history_bootstrap=0`) — a human still pre-builds every profile / (b) **default ON** with conservative thresholds (`coding_history_consensus_threshold≈0.8`, `coding_history_min_samples≈3`, `coding_history_lookback≈10`) so routine vendors self-code from their own posted history and only genuinely ambiguous/no-history cases escalate / (c) ON + auto-self-seed a profile (`coding_source="ap-coding-history-v1"`) so the cheaper Layer-1 lookup takes over next time | **(b), trending to (c)** — closes the [[00-overview]] row-06 gap with the Coding-Review escalation seam intact; the consensus + min-sample gate prevents auto-posting on shaky history. Self-seeding (c) is the natural follow-on once (b) proves out. Keep the as-built profile-only path documented; bootstrap is a planned additive layer (AC-06-15..18). | Russ + Bryan | Before the coding-bootstrap slice (sets the Settings defaults + the consensus math) |
