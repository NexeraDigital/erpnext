...
Permissions: System Manager rw; Accounts Manager read; Accounts User read; `Auditor (Read Only)` (new role, bible) read. Rows are written server-side (`ignore_permissions=True`) by the scheduler / on-submit hook only. **Open Decision D3**: this dedicated cache DocType vs a JSON field — recommend the DocType for queryability and a clean UNIQUE-on-supplier guarantee.
...
5. **Compute the invoiced quantities/rates.** Prefer the promoted Purchase Invoice's items (each carries `po_detail` linking the PO Item — verified join key C6) when the capture is already promoted; else use `capture.line_items` qty/rate; else (header-only fallback) treat the whole invoice as one line at qty = 1, rate = `final_total_amount` against the single header PO. (This is why 3WM may run **pre- or post-promotion** — see §5.4 / D5.)
...
- **D3 — Anomaly baseline cache: dedicated DocType vs JSON field (MEDIUM).** **Recommended default: dedicated `AP Supplier Anomaly Baseline` DocType** (queryable, UNIQUE-on-supplier, cheap upsert). **Owner:** spec 08 author. **Lock:** before the cache is created.
...
- **D5 — 3WM pre- vs post-promotion ordering (HIGH).** Blocking 3WM must run **before** promotion, but the most authoritative join (`po_detail`) exists only **after** promotion. **Options:** (a) run 3WM pre-promotion off `capture.line_items.po_reference` (from spec 04) + header PO, then re-affirm post-promotion off `po_detail`; (b) run only post-promotion and block at the *approval* gate instead of promotion. **Recommended default: (a)** — pre-promotion block off the capture's own PO references (header + line) keeps the gate where the plan puts it (step 7, before payment), and a post-promotion re-affirmation tightens it. **Owner:** spec 08 + [[07-classification-doctype-branching]] (ordering). **Lock:** before wiring into `validate_for_purchase_invoice`.
