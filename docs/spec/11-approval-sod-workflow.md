...
| Pending Review | Auto-Approve | Approved | `AP Clerk` | `doc.grand_total <= 1000` | 1 *(under-threshold; submitter may self-advance)* |
| Pending Review | Send for Approval | Pending Approval | `AP Clerk` | `doc.grand_total > 1000` | 1 |
| Pending Approval | Approve | Approved | `Accounts Manager` | `doc.grand_total > 1000` | **0** |
| Pending Approval | Reject | Draft | `Accounts Manager` | *(blank)* | **0** |
| Approved | Submit | Submitted | `Accounts Manager` | *(blank)* | **0** |

Condition strings use only the verified `safe_eval` whitelist. Department / cost-center / supplier-risk routing extends the Condition, e.g. `doc.grand_total > 1000 and doc.cost_center == 'OPS-01'` or supplier-risk via `frappe.db.get_value('Supplier', doc.supplier, 'custom_risk') == 'High'` (the `custom_risk` field is owned by [[08-validation-gates]] / [[05-supplier-resolution]]).
