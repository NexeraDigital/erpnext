...
| D4 | Who owns the new `journal_entry` link field on `AP Invoice Capture` (needed for Stream R deposit clears). | (a) [[07-classification-doctype-branching]]; (b) this spec. | **(a)** — owned by 07; this spec consumes it and adds it only if 07 hasn't landed. | Spec 07 / this spec | Before Stream R clearing is testable. |
