...
> _Revised 2026-06-02: re-visioned automation-first ([[00-overview]] "Guiding principle"). §1 and §5.3 now lead with native auto-match → auto-clear on the confident case (no human matching); the only human touch is the **residual** (unmatched past SLA, amount mismatch, duplicate). The default live feed is **native Plaid** (zero connector code) unless a concrete reason for SimpleFIN is recorded (§8 D8). The dev mock + writeback are provider-agnostic and unchanged._

> **North-star alignment ([[00-overview]]).** Closing the loop should be hands-free: the bank feed **auto-matches** postings to bank lines using ERPNext's **native** reconciliation engine (no human matching on the confident case), and a clean match **auto-clears** the document. A human is pulled in **only** for the **residual** — an unmatched line past the SLA, an amount mismatch, a possible duplicate. The design choice is therefore **auto-match + escalate-the-residual**, never a manual reconciliation step on the happy path. Per [[00-overview]] decision #8 the default live feed is the **native Plaid** feed (zero connector code — auto-sync produces the `Bank Transaction` rows for free) unless a concrete business reason for SimpleFIN is recorded; less connector code is more automation for free.

## 1. Summary
This spec implements **Step 12** of `docs/planning/workflow-v2-plan.md`: the **external** bank-feed signal that actually closes an AP transaction — **automation-first**. It **reuses ERPNext's native Bank Reconciliation engine to auto-match** bank lines to postings (it does **not** rebuild matching), and on a confident match it **auto-clears** the document by writing a clearance result back onto `Document Capture` via two new fields (`bank_transaction`, `bank_cleared`) — **no human matches on the happy path.** The only human touch is the **residual**: an unmatched line past the 72h SLA, an amount mismatch, or a possible duplicate, which is surfaced for review. It serves **both streams**: deposit (card) bank lines clear Stream R already-paid **Journal Entries**; withdrawal (ACH/check) bank lines clear Stream I **Payment Entries**. The **default live feed is the native Plaid integration** (zero connector code — it already produces the `Bank Transaction` rows this spec reconciles) unless a concrete reason for SimpleFIN is recorded (§8 D8); during development the feed is mocked by provider-agnostic fixtures so build + tests don't wait on a live feed. The one-line current-state delta: today closure is a single derived `closed` boolean with an active guardrail asserting **zero** Bank Transactions; this spec **retires that guardrail** and splits closure into `settled` (today's internal derivation) + `bank_cleared` (the new external half).
...
1. **Closure is DERIVED, not stored, and the active guardrail is "no Bank Transaction."** Two parallel implementations:
   ...
   - **`erpnext/accounts/doctype/document_capture/document_capture.py`** (production controller):
     - Module docstring `document_capture.py:9-10`: *"must not progress silently into Purchase Invoice, Payment Entry, or Bank Transaction."* — **second docstring to update in the same PR.**
     - `_bank_transaction_count_for_payment_entry(payment_entry)` `document_capture.py:1393-1399`: same count helper.
     - `_derive_payment_lifecycle_status(capture)` `document_capture.py:1402-1423`: returns `Blocked / Not Requested / Closed / Confirmed`; `Closed` uses the same 4-condition test (`document_capture.py:1415-1422`). **This is where the new `bank_cleared` terminal sub-state slots.**
     - `issue_mock_payment(...)` `document_capture.py:1426-1504`: creates+submits a native PE via `get_payment_entry`, writes `mock_payment_*` fields, gated by `frappe.only_for(MANAGER_APPROVAL_ROLE_DEFAULT)` + `pe.insert(ignore_permissions=True)`. Response dict includes `bank_transaction_count` (`document_capture.py:1487`).
     - `build_closure_evidence(capture)` `document_capture.py:1523-1631`: computes `closed` identically (`document_capture.py:1551-1558`); `native.bank_transaction_count` at `document_capture.py:1623`; `closure_basis` at `document_capture.py:1626-1630` **literally asserts "Bank Transaction count must remain 0" — this string is inverted by this spec.**
...
3. **`Document Capture` JSON already has** `purchase_invoice` (Link, read-only, idx 53) and `payment_entry` (Link, read-only, idx 67) plus a `mock_payment_*` section (idx 66-75). It does **not** have `bank_transaction` or `bank_cleared` — confirmed via JSON parse (`has bank_transaction field? False`, `has bank_cleared field? False`). `autoname: format:APIC-{YYYY}-{#####}`, `track_changes: 1`.
...
|---|---|---|---|
| 1 | https://docs.frappe.io/erpnext/user/manual/en/bank-reconciliation | Matching engine is **RANK-based** (rank = amount + reference-number + party, each +1; **date is a *filter* on the candidate set, not a rank signal** — confirmed in source, §5.3-source); party-name is a fuzzy **hint**, not a clearance; manual reconciliation sets `Clearance Date`. Grounds "exact-amount PRIMARY (amount + reference-number rank; date filters), party-name HINT." | "The vouchers that are related to this transaction will be displayed. They will be ranked on the basis of the maximum number of fields matched." Also: "a party is auto set (if matched) in the Bank Transaction" via "fuzzy matching capabilities for approximate name matching." |
...
- `get_matching_queries(...)` contract — `erpnext/accounts/doctype/bank_reconciliation_tool/bank_reconciliation_tool.py:628-684`:
  ...
  ```
  Routing within it (verified `bank_reconciliation_tool.py:645-682`): `"payment_entry" in document_types` and `"journal_entry" in document_types` always considered; `transaction.deposit > 0.0 and "sales_invoice"` adds SI; `transaction.withdrawal > 0.0` + `"purchase_invoice"` adds PI. `exact_match` filters on amount equality; the rank score is `amount_rank + ref_rank + party_rank + 1` (amount, **reference number**, and party each contribute +1), while **posting/reference date bounds the candidate set as a filter, not a rank component** — so amount is the gate, reference-number is a co-primary rank field, and party is a weak hint. (Refines the plan's "amount + date primary" shorthand: date *filters*, it does not *rank*.)
...
#### New fields on `Document Capture` (mirror existing read-only `mock_payment_*` convention; place after `payment_entry`, idx 67)
...
**Permissions.** No new doctype, so no new permission rows for the feed. The three new `Document Capture` fields are `read_only=1` (writeback-only) and inherit the existing capture permission set. The whitelisted replay wrapper (§5.2) is gated to `MANAGER_APPROVAL_ROLE_DEFAULT` ("Accounts Manager") to match the existing payment-issuance authorization boundary — see [[11-approval-sod-workflow]] for the role model and the new `Treasury Approver` role this may migrate to.

### 5.2 Endpoints

All whitelisted methods live in `erpnext/accounts/doctype/document_capture/document_capture.py` and follow the existing string/dict-normalization convention of the `*_for` wrappers (`document_capture.py:1677-1759`): accept `capture: str`, coerce JSON-string args, call `_kick_next_step()` after a state change.
...
**Native reuse + auto-match → auto-clear → escalate-the-residual (the automation-first shape).** This spec reuses ERPNext's native reconciliation engine wholesale to **auto-match** the confident case: matching is performed by the native `Bank Reconciliation Tool.get_matching_queries` accumulate hook (`erpnext/accounts/doctype/bank_reconciliation_tool/bank_reconciliation_tool.py:628`, registered via `hooks.py`), and the PE/JE match surface is the native `Bank Transaction Payments` child (the `payment_document`/`payment_entry` Dynamic-Link join, §4). A clean (exact-amount, in-window) match **auto-clears** the capture via the writeback below — **no human matches on the happy path.** A human is pulled in **only for the residual** (unmatched past the SLA, amount mismatch, or possible duplicate), surfaced via the SLA-breach rows (§5.3-G) to [[10-ap-review-observability]]. No matching logic, no match-storage doctype, and no reconciliation UI are rebuilt here; the fork adds **only** the post-reconciliation writeback (below) and an optional ranking-bias method behind Decision D1 (default: not built). Everything else is native — including, by default, the **Plaid** feed that produces the bank lines (D8).
...
doc_events = {
    "Bank Transaction": {
        "on_submit": "erpnext.accounts.doctype.document_capture.document_capture.ingest_bank_transaction_match_for",
        "on_update_after_submit": "erpnext.accounts.doctype.document_capture.document_capture.ingest_bank_transaction_match_for",
        "on_cancel": "erpnext.accounts.doctype.document_capture.document_capture.revert_bank_transaction_match_for",
    },
}
```
...
- **`_kick_next_step` symmetry:** the existing `issue_mock_payment_for` wrapper already calls `_kick_next_step()` "for symmetry … so any future hop … plugs in" (`document_capture.py:1739-1743`). The writeback path reuses `_kick_next_step()` post-write so a future closure-evidence-auto-generation hop ([[14-closure-audit-retention]]) plugs in without changing this wrapper.
...
- **Paired-doc obligations (CLAUDE.md).** Changes under `erpnext/accounts/ap_closed_loop/` and `.../document_capture/` require `docs/architecture/FORK-CHANGES.md` AND `FORK-CHANGES-PLAIN.md` updates in the same commit. Write an **ADR** locking the guardrail-retirement decision (settled vs bank_cleared split).

## 6. Acceptance criteria

- **AC-13-1 (fields exist).** `Document Capture` gains `bank_transaction` (Link→Bank Transaction, read-only), `bank_cleared` (Check, read-only), `bank_cleared_at` (Datetime, read-only); `payment_lifecycle_status` gains a `Bank Cleared` option.
...
- `erpnext/accounts/doctype/document_capture/test_document_capture.py:1192` `test_closure_evidence_reconstructs_full_lifecycle` — replace `native.bank_transaction_count == 0` (`:1208`) with the dual-state assertion (AC-13-7).
- **Left intentionally green (per ADR scope):** `test_issue_mock_payment_does_not_create_bank_transaction` (`test_document_capture.py:1123`) and the `response["bank_transaction_count"] == 0` assertion (`:1075`) — the FEED creates BTs, the mock PE does not (AC-13-13). Confirm the ADR scopes "guardrail retired" to the closure-evidence assertions only.
