...
> **What flows with no human:** the classifier reads the confirmed extraction and auto-routes the document to its posting doctype — content-paid → Already-Paid posting, plain bill → Purchase Invoice — and **silently confirms-or-corrects** the Step-1 intake stream tag. A weak intake guess that disagrees with a *confident* content read is **overridden by the content and recorded as telemetry** (an `AP Review Event` / `stream_tag_agreement` signal for [[10-ap-review-observability]]'s tuning loop) — not bounced to a human.
> **The single escalation seam:** Manual Review fires only when the **content itself is ambiguous** (the document genuinely can't be classified), never merely because intake and content disagree. The Employee-Reimbursement → Manual Review landing is a **deferral, not a design choice** — hrms is absent on this bench, so the branch parks; it auto-routes to Expense Claim the moment hrms lands (§8 D-07-3).
> **Automation to grow into:** trust a confident content classification over a weak intake tag (the planned ACs below), shrinking the disagreement-driven Manual-Review tail toward only the truly-ambiguous documents. Per [[00-overview]] "Automation-first doctrine" row 07: *stop sending **every** stream disagreement to Manual Review.*
...
> [!note] Planned (2026-06-02) — automation-first: trust confident content over a weak intake tag
> **Shipped behavior (above) sends *every* intake-vs-content disagreement to Manual Review.** That over-escalates: content (the structured extraction) is usually the *stronger* signal, and intake's Step-1 tag is a fast heuristic guess. The planned design replaces the unconditional "disagree → Manual Review" with a confidence test:
> - **Content is confident** (the classifier resolved a clear marker — e.g. an explicit `PAID` / last-4 card marker for Already-Paid, or a normal non-employee supplier for Unpaid Bill — and the per-field confidence from [[04-extraction-confidence-line-items]] for the driving fields is above threshold): **trust the content**, set `document_type`/`classified_stream` from the content read, set `stream_tag_agreement = 'Disagree'`, **silently record the correction as telemetry** (`AP Review Event`, root-cause `stream_mistag`) for spec 10's tuning loop, and **continue the cascade with no human pause**. The intake heuristic was simply wrong and the disagreement rate is the tuning signal — not a reason to stop the document.
> - **Content is ambiguous** (genuinely unclear — no decisive marker, an ambiguous extraction, or the confident-content test fails): **escalate to Manual Review** exactly as today. This is the real exception the seam exists for.
> The distinction is *content ambiguity*, not *disagreement-with-intake*. Disagreement alone is telemetry; only genuine ambiguity pauses a human. Implement behind the existing `classify_document_type` step-5 branch (a confidence gate on the content read before the override-to-review). The `stream_tag_agreement` signal and the `AP Review Event` emission are unchanged — only whether the document **also** halts. *(Status stays `Done` for the shipped behavior; this is a planned extension — see the planned ACs AC-07-15 / AC-07-16.)*
...
- **AC-07-15 (planned — confident content beats weak intake, no human):** A capture with `provisional_stream='I'` but a **confident** content read (an explicit PAID / last-4 card marker, driving-field confidence above threshold) classifies to `document_type='Already Paid'`, `classified_stream='R'`, `stream_tag_agreement='Disagree'`, **does NOT halt** (`action_required==0`, `status != 'Manual Review'`, cascade continues), and emits exactly one `AP Review Event` (root-cause `stream_mistag`) recording the silent correction. *(Planned — automation-first extension; not yet built.)*
- **AC-07-16 (planned — ambiguous content still escalates):** A capture whose **content is genuinely ambiguous** (no decisive marker / extraction below the confidence threshold) — whether or not it disagrees with intake — classifies to `document_type='Manual Review'`, `status='Manual Review'`, `action_required==1` with an ambiguity reason. (Confirms the seam still fires on real ambiguity, not on mere disagreement.) *(Planned — automation-first extension; not yet built.)*
...
- **D-07-5 — Classifier fallback when supplier unmatched (employee check needed).** Options: default to **`Manual Review`** (safe — don't guess), vs default to **`Unpaid Bill`** (Stream I, optimistic). **Recommended default: `Manual Review`** (an unmatched supplier on Stream I already blocks at validation per plan Step 4 line 39, so routing to review is consistent). **Owner:** accounting lead. **Lock by:** before AC-07-6 test is written. *(Automation-first nudge, 2026-06-02: this remains `Manual Review` because an unmatched supplier is genuine ambiguity, not mere intake disagreement — consistent with the planned stance below. The automation-first change targets the **disagreement** path, not this one: see the §5.3 "Planned (2026-06-02)" note — a **confident** content read that merely disagrees with a weak intake tag is trusted-and-recorded, not escalated; only ambiguous content like this unmatched-supplier case reaches Manual Review. Original `Manual Review` default kept.)*

- **D-07-8 — Disagreement routing: confident content vs unconditional Manual Review (planned, 2026-06-02).** Shipped behavior routes **every** intake-vs-content stream disagreement to Manual Review (§5.3 step 5). Options: **(a)** keep unconditional escalation (shipped); **(b)** when the **content read is confident**, trust it over the weak intake tag, record the correction as `stream_tag_agreement='Disagree'` telemetry (`AP Review Event`, `stream_mistag`), and **continue with no human** — escalating only when the **content itself is ambiguous**. **Recommended default (automation-first): (b)** — content is the stronger signal; disagreement is a tuning signal, not an exception (see [[00-overview]] "Automation-first doctrine" row 07, and the §5.3 planned note + AC-07-15/AC-07-16). **Owner:** accounting lead + spec 09 (confidence threshold source). **Lock by:** when the auto-confirm / confidence-gating slice (overview #9/#4) is built — this rides the same per-field-confidence signal.
