...
## 1. Summary
This spec adds the Step-6 fork that classifies a confirmed capture into an ERPNext document type and routes it to the correct posting doctype: **Unpaid Bill → Purchase Invoice** (Stream I, existing path), **Already Paid → Journal Entry** (Stream R, NEW path: DR expense / CR card-or-cash liability), **Employee Reimbursement → Expense Claim** (Stream R-employee, DEFERRED — hrms absent, routed to Manual Review), **Other / stream conflict → Manual Review**. It is the heart of correct ERPNext AP automation: routing every document to one doctype double-counts liabilities and breaks trial-balance reconciliation. Current-state delta: today the pipeline is hardwired to a single doctype (Purchase Invoice) with no `document_type` field, no stream tag, and no Journal Entry path; this spec inserts a classification hop into the auto-progression cascade and forks on the result.
...
- **Stream R (Already Paid):** must **NOT** pass through `validate_for_purchase_invoice` (that function is PI-specific — it matches a Supplier and classifies PO/PR). Instead it routes to a NEW `promote_to_journal_entry` that posts DR expense / CR card-or-cash liability with `voucher_type='Credit Card Entry'` (or `'Cash Entry'`). No approval routing, no payment execution (the money already moved); closure is reconciliation-only (extends in [[14-closure-audit-retention]]).
...
The Already-Paid posting is built inside a **single swap-friendly helper** so the §8 D-07-1 A↔B↔C decision is one function change:

```python
def _build_already_paid_voucher(capture, defaults) -> "Document":
    """Single decision point for the already-paid posting.
    Default impl (option A): build a balanced Journal Entry.
    Swappable to option B (PI is_paid=1) or C (PI + clearing account)
    without touching promote_to_journal_entry's callers."""
```
...
- **Closure ([[14-closure-audit-retention]]):** the `journal_entry` link + `voucher_type` is the NEW Stream-R closure anchor — `derive_closure_evidence` (walking_skeleton.py:278-339) must learn to recognise a submitted JE (and its bank-feed match from [[13-bank-feed-reconciliation]]) the way it currently recognises PI+PE.
...
- **D-07-1 — Already-Paid posting mechanism.** Options: **(A)** direct **Journal Entry** (DR expense / CR card-or-cash liability) — plan default, fewest docs; **(B)** **Purchase Invoice with `is_paid=1`** (native DR Supplier / CR Bank-Cash per the verified PI "Is Paid" citation — vendor shows in AP aging momentarily then nets to zero); **(C)** **Purchase Invoice + a 'Credit Card Clearing' account** (most docs, but vendor shows in AP aging *and* spend-by-supplier). **Recommended default: (A).** Implement behind the single `_build_already_paid_voucher()` helper so A↔B↔C is a one-function swap. *(Note: option B was surfaced by the upstream PI docs and was NOT in the original guidance — it is the lightest way to keep vendor visibility.)* **Owner:** accounting lead (Bryan). **Lock by:** start of the build for this spec.
