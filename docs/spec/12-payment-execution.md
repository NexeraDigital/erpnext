...
| 2 | https://docs.frappe.io/erpnext/user/manual/en/payment-order | **Payment Order is the native ERPNext batch wrapper for bulk supplier disbursement** — the correct anchor for the phase-2 real ACH/check rail. **It does NOT document ACH, NACHA, or bank-file generation**, confirming real-rail file generation is **not** a built-in ERPNext capability and must be a fork/integration concern (validates the phase-2 "does the bank support ACH origination" open question). | "A Payment Order is an internal document to record bulk payments against Suppliers." |
...
#### (A) NEW Custom Field on `Supplier` — `auto_pay_eligible` (per-supplier gate, default OFF)
Shipped as a **Custom Field fixture** (not a Supplier core JSON edit), so it travels with the app. This establishes the first Supplier Custom Field fixture in the fork.
...
Delivery: add a `fixtures` entry to `erpnext/hooks.py` (net-new key — see §3-K) filtered to this Custom Field, e.g. `{"dt": "Custom Field", "filters": [["name", "in", ["Supplier-auto_pay_eligible"]]]}`, and export the fixture JSON under `erpnext/accounts/` (the AP closed-loop module). **Alternative home (noted, not chosen for phase-1):** a field on a per-supplier "coding profile" doctype if [[06-gl-coding-tax-costcenter]] introduces one — rejected for phase-1 to avoid a new cross-spec dependency; the Supplier Custom Field is self-contained.

#### (B) NEW field on `AP Invoice Capture` — `payment_rail_used` (audit, written at issuance)
| fieldname | fieldtype | options / default | purpose |
|---|---|---|---|
...
Today the Literal is `["Not Requested", "Confirmed", "Closed", "Blocked"]` (`ap_invoice_capture.py:232-234`). For real (async) rails the PE is *recorded* (`"Confirmed"`) before it is *bank-matched*. This spec **adds `"Bank Cleared"`** so the enum distinguishes:
...
**(4) Cascade gate — the phase-1 correctness fix (`_determine_next_step` Step 4, :357-364).** Replace the unconditional Step-4 auto-pay with a **stream-aware, `auto_pay_eligible`-gated** branch:
...
- **Insertion point:** `_determine_next_step` Step 4 (`ap_invoice_capture.py:357-364`) is replaced by the stream-aware, gated branch in §5.3-(4). This is the ONLY cascade change.
...
### 7.1 Automated
Extend `erpnext/accounts/doctype/ap_invoice_capture/test_ap_invoice_capture.py` (base `from frappe.tests import IntegrationTestCase`; `frappe.db.rollback()` in `tearDown`; mirror the existing payment suite at :1054-1153). Run:
...
- [[01-foundations-settings-async-idempotency]] — the shared `with_idempotency` + `AP Posting Ledger` + `idempotency.generate_key` primitive for the PE insert. Today's code has only the existence guard (`ap_invoice_capture.py:1449`). **Fallback** if it slips: per-capture `payment_idempotency_key` field (D-12-5).
...
- `docs/architecture/FORK-CHANGES.md` + `docs/architecture/FORK-CHANGES-PLAIN.md` MUST be updated in the same commit (change touches `ap_invoice_capture/` scope + adds a Supplier Custom Field) — paired per CLAUDE.md.
