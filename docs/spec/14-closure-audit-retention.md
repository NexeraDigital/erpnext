...
**Net-new (confirmed absent — grep across `erpnext/` found zero AP-scoped hits):** no `build_audit_trail` / `build_audit_trail_for`; no `AP Audit Trail` Print Format; no `Accounts Payable Trial Balance Drift` report (existing TB reports are `trial_balance`, `consolidated_trial_balance`, `trial_balance_for_party`, `trial_balance_simple` under `erpnext/accounts/report/`); no `archive_pending` field; no 7-year retention scheduler (the only "retention" hit is the unrelated MCP pruner `erpnext/mcp/tasks.py`, registered at `hooks.py:461`).
...
| C-9 | `erpnext/accounts/doctype/bank_transaction/bank_transaction.json` (in-repo) | Bank Transaction fields available to the bank-match block: `date` (Date), `status` (Select), `bank_account` (Link), `reference_number` (Small Text), `deposit`/`withdrawal`/`allocated_amount` (Currency). `reconciled_by`/`reconciled_at` come from the doc's owner/modified or a [[13-bank-feed-reconciliation]]-added field. | Re-confirm which field [[13-bank-feed-reconciliation]] uses for `reconciled_by`/`reconciled_at`. |
...
**A) Changed field — `payment_lifecycle_status`** (existing Select, `ap_invoice_capture.json:577-581`):

| fieldname | fieldtype | options / default | purpose |
|---|---|---|---|
| `payment_lifecycle_status` | Select | `Not Requested\nConfirmed\nClosed\nClosed (Settled)\nClosed (Bank Cleared)\nBlocked` (default `Not Requested`, `read_only`, `in_standard_filter`) | Adds the two dual-signal terminals **alongside** the existing `Closed` (kept as a back-compat alias). `_derive_payment_lifecycle_status` returns the most-specific applicable value. JSON Select-option add only = **test-exempt** per CLAUDE.md; the derivation change is **not** exempt. |
...
3. Read parent `Bank Transaction`: `date`, and `reconciled_by`/`reconciled_at` (from the field [[13-bank-feed-reconciliation]] defines, or fall back to BT `modified_by`/`modified`).
...
4. `if bank_cleared: return "Closed (Bank Cleared)"`; `elif settled: return "Closed (Settled)"`; `else: return PAYMENT_LIFECYCLE_CONFIRMED`.
5. Keep `PAYMENT_LIFECYCLE_CLOSED == "Closed"` constant defined as a back-compat alias (no longer the primary return value).
...
- **Dual-signal terminal:** the capture's terminal `payment_lifecycle_status` advances **passively** from `Closed (Settled)` → `Closed (Bank Cleared)` when [[13-bank-feed-reconciliation]] lands the Bank Transaction match. There is no enqueue here — the value is **derived** each time evidence is built (consistent with the "no custom closed flag" design).
...
- **AC-14-5 (dual closure, settled-only, positive):** PI Paid + PE submitted, **no** Bank Transaction match → `settled == True`, `bank_cleared == False`, `payment_lifecycle_status == "Closed (Settled)"`, and `closure_basis` mentions **both** signals and does **not** contain "must remain 0".
- **AC-14-6 (dual closure, bank-cleared, Stream I, positive):** a Bank Transaction matched to the PE (a `Bank Transaction Payments` row) → `bank_cleared == True`, `bank_match` populated with `{bank_transaction, date, amount, reconciled_by, reconciled_at}`, `payment_lifecycle_status == "Closed (Bank Cleared)"`.
...
- **UPDATE `test_closure_evidence_reconstructs_full_lifecycle`** (`:1192-1210`) *(AC-14-5)* — change `assertEqual(...bank_transaction_count, 0)` and `assertIn("No custom closed flag", closure_basis)`; assert `settled is True`, `bank_cleared is False`, lifecycle `"Closed (Settled)"`, and `closure_basis` mentions both signals.
- `test_dual_closure_bank_cleared_stream_i` *(positive, AC-14-6)* — seed a `Bank Transaction` + `Bank Transaction Payments` row for the PE → `bank_cleared`, `bank_match` populated, lifecycle `"Closed (Bank Cleared)"`.
...
- **D-14-1 — `payment_lifecycle_status` migration vs alias.** Options: (a) keep `Closed` + add the two new values, derivation returns the most specific; (b) migrate all to the two new values and drop `Closed`. **Recommended default: (a) alias-keep** — avoids breaking the `PAYMENT_LIFECYCLE_CLOSED` constant (`ap_invoice_capture.py:90`) and the 5 existing assertions. Owner: AP closed-loop lead. Lock: before the JSON Select edit lands (build start). Verify v16 patch conventions before writing any data patch.
...
- **D-14-5 — `reconciled_by`/`reconciled_at` source.** Options: (a) a field [[13-bank-feed-reconciliation]] adds to Bank Transaction; (b) fall back to BT `modified_by`/`modified`. **Recommended: (a) if [[13-bank-feed-reconciliation]] defines it, else (b).** Owner: bank-feed-spec owner. Lock: when [[13-bank-feed-reconciliation]] data model is finalized.
