...
> _Revised 2026-06-02: re-visioned automation-first ([[00-overview]] "Guiding principle"). §1 and §5.4 now state plainly that closure is **derived, never declared** — no human "mark closed" step exists; `payment_lifecycle_status` flips to `Bank Cleared` passively the moment `settled` + `bank_cleared` agree. The audit trail, TB-drift report, and 7-year retention flag are **automated on-demand / scheduled outputs**; the only human touchpoint is a **surfaced exception** (TB drift to investigate)._

> **North-star alignment ([[00-overview]]).** Closure is **derived, not declared** — there is **no human "mark as closed" step anywhere**, by design. A capture's terminal `payment_lifecycle_status` flips to `Bank Cleared` **passively**, the moment both signals agree (`settled` AND `bank_cleared`), re-derived each time evidence is built — no flag is hand-set and no button is clicked. The audit trail (`build_audit_trail_for`), the TB-drift report, and the 7-year retention flag are **automated outputs** — assembled on demand or run on the scheduler — so audit readiness costs **no routine human effort**. A human is pulled in **only** for a genuine exception the automation *surfaces* (e.g. a trial-balance drift over threshold to investigate); closure itself, and the audit chain it produces, stay hands-free.

## 1. Summary
This spec closes the loop **automatically** — closure is **derived, never declared.** It (a) upgrades the closure signal from single (PI Paid + PE submitted) to **dual** — `settled` plus `bank_cleared` — by reading the Bank Transaction match landed by [[13-bank-feed-reconciliation]], so a capture's terminal `payment_lifecycle_status` flips to `Bank Cleared` **passively the moment both signals agree, with no human "mark closed" step**; (b) adds **single-record audit retrieval** (`build_audit_trail_for`) that assembles the live closure evidence + a Frappe-`Version` field-level history block + a validation-gates summary + a bank-match block into one composite dict on demand and renders it to one PDF via an `AP Audit Trail` Print Format — an **automated output**, not a human-assembled binder; (c) ships an **`Accounts Payable Trial Balance Drift`** report comparing current Creditors / Credit-Card-Clearing balances to the prior-period `Account Closing Balance` snapshot — the **one surface that pulls a human in**, and only on an over-threshold drift to investigate; and (d) installs a **flag-only 7-year retention scheduler** (IRS — never deletes), run automatically on the daily scheduler. It is stream-agnostic in structure, but `closure_basis` differs by stream: Stream R closes on the bank-match of the Journal Entry, Stream I on the bank-match of the Payment Entry. **Current-state delta:** today closure is single-signal and `closure_basis` asserts "Bank Transaction count must remain 0" — this spec rewrites that to require a Bank Transaction for the bank-cleared leg, and the closed state is re-derived each time rather than stored.
