...
def generate_key(capture_name: str, step_name: str) -> str
    # sha256 hex of NUL-joined [capture_name, step_name, str(settings_modified_version)]
...
def _dispatch_step(capture: str, step_name: str, _attempt: int = 0, **kwargs) -> None
    # worker entry point: retry/backoff + dead-letter (NOT whitelisted)
```
...
1. Read the Single's version: `version = frappe.db.get_single_value("AP Closed Loop Settings", "modified")` (cheap single-field lookup). A settings change bumps `modified`, invalidating prior keys (per the guidance "settings modified-version").
2. Build the key material as a list `[capture_name, step_name, str(version)]`.
3. Join with a NUL byte `"\x00"` (record-separator) — **not** a printable delimiter — so a name containing the separator can't forge a collision.
4. Return `hashlib.sha256(material.encode()).hexdigest()` (64-char hex).
- No persistence, no exceptions raised here; pure function.
...
4. `_dispatch_step(capture, step_name, _attempt=0, **kwargs)` is the worker entry point:
   ...
   b. `try: fn(capture)`.
   c. On exception, classify: **transient** = DB lock/deadlock (already retried by frappe's `execute_job`, so usually surfaces only after frappe's own retries) OR provider 429/5xx OR explicit transient marker. **permanent** = `frappe.ValidationError` and its AP subclasses (`CapturePromotionError`, etc.), bad data, missing config.
   d. **Transient + `_attempt < MAX_RETRIES (3)`:** re-enqueue via `enqueue_step(capture, step_name, _attempt=_attempt+1, ...)` with backoff `delay = base * 2**_attempt`. **Realizing the delay** is an Open Decision (D5): `frappe.enqueue` has no delay arg. Recommended default: a `next_retry_at` Datetime stashed on the capture (or a tiny retry-queue row) plus a `scheduler_events.all`/`cron` sweep that re-enqueues due retries — this avoids blocking a worker on `sleep`. Simpler interim: immediate re-enqueue without true delay (accept tighter retry spacing) for phase-1.
...
- **AC-01-1** (positive): `generate_key(c, s)` returns the same 64-char hex on repeated calls for the same `(capture, step)` while `AP Closed Loop Settings.modified` is unchanged.
- **AC-01-2** (edge): changing the `step`, the `capture`, OR saving `AP Closed Loop Settings` (which bumps `modified`) yields a different `generate_key` result.
...
- **`erpnext.accounts.ap_closed_loop.tests.test_idempotency`** (new file `erpnext/accounts/ap_closed_loop/tests/test_idempotency.py`):
  - `generate_key` determinism (positive — AC-01-1); sensitivity to step/capture/settings-save (edge — AC-01-2).
...
- **`erpnext.accounts.ap_closed_loop.tests.test_async_runner`** (new file `.../tests/test_async_runner.py`):
  ...
  - retry: transient-failing step re-enqueues with `_attempt+1` up to `MAX_RETRIES`, then stops; assert attempt count (edge — AC-01-9).
