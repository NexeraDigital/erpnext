...
# VENDOR PATCH (see _vendor/PROVENANCE.md): jsonschema import made lazy. Upstream
# imports it at module top, but it is only used by run_tool() — which the ERPNext
# MCP server never calls (handle_call_tool validates via Pydantic instead). Keeping
# it lazy avoids a runtime dependency on a package not in the bench env.
...
def run_tool(tool: Tool, arguments: dict[str, Any]):
    from jsonschema import validate  # VENDOR PATCH: lazy import (see module top)
