...
are written exclusively by ``emit_review_event`` in the Document Capture
...
class APReviewEvent(Document):
	...
	if TYPE_CHECKING:
		...
		action_taken: DF.Literal["field_corrected", "supplier_created", "rejected", "classified_other", "coding_completed"]
		capture: DF.Link
		clerk: DF.Link | None
		...
		root_cause_tag: DF.Literal["", "extraction_miss", "supplier_unmapped", "confidence_threshold_too_tight", "stream_mistag", "policy_violation", "vendor_error", "missing_po", "other"]
