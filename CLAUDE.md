...
- **This fork runs Frappe framework v16** (currently `16.18.3`, branch `russ/migrateToV16`). The published framework **user-docs have no v16 tree yet** (`…/framework/v16/…` 404s) — so for anything version-sensitive, the **v16 source is authoritative, not the docs**.
- **Canonical sources, in priority order:**
  1. **The installed tree at `apps/frappe` (v16) + `https://github.com/frappe/frappe` on the v16 branch/tag** — SOURCE OF TRUTH for API signatures, hook names, DocType field types, permission semantics, background jobs, and bench commands on v16. The code you run is the most authoritative check there is — grep the installed source before trusting any doc URL. Cite the file + line.
  2. `https://docs.frappe.io/framework/v15/user/en/` — conceptual reference for framework concepts that are **stable across v15→v16** (DocTypes, hooks model, controllers, permissions, client scripts, the bench mental model). Append the section path, e.g. `.../basics/doctypes`, `.../python-api/hooks`. The bare `/framework` URL is a landing page — skip it. **MAY LAG v16** — verify any concrete signature / field-type / argument against the v16 source (item 1) before relying on it.
...
- **For changes to the AP capture cascade / workflow** — any edit that changes the flow the sequence diagram depicts. **Read this trigger broadly:** it fires not only on a new/removed/reordered cascade hop, a renamed step function, or a new actor/participant, but on **any new or changed branch, gate, guard, pause seam, park, STOP, or block reason inside an existing step** — even when `_determine_next_step` and the async routing are untouched. Concretely, this includes:
  ...
  - edits to **any** whitelisted step function — currently `run_dedupe_for` / `run_fake_extraction_for` / `classify_document_type_for` / `validate_for_purchase_invoice_for` / `promote_already_paid_for` / `apply_coding_profile_for_ui` / `request_approval_for` / `issue_mock_payment_for` (keep this list current as steps are added);
  - **a new validation gate, promotion guard, classification branch, or block/park/STOP condition added inside an existing step** (e.g. spec 08 added three gates inside `validate_for_purchase_invoice` and a bank-change re-check inside `promote_to_purchase_invoice` — both require a diagram update even though no hop was added).

  If in doubt whether your change is "flow-visible," assume it is and refresh the diagram. **At every spec slice that touches the cascade, re-read `AP-CAPTURE-SEQUENCE.md` against the code and confirm it still matches before marking the spec done** — a slice that changes the flow but leaves the diagram stale is **not done** (specs 06 and 07 were missed this way; don't repeat it). **In the same commit you MUST:**
